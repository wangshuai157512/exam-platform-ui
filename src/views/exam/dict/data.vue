...
import {
  ...
  updateData,
  exportData,
} from "@/api/exam/dict/data";
import { listType, getType } from "@/api/exam/dict/type";
export default {
  name: "Data",
  data() {
    return {
  ...
  methods: {
    ...
    /** 查询字典类型列表 */
    getTypeList() {
      listType({ pageNum: 1, pageSize: 20 }).then((response) => {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.id != undefined) {
            const { id, status, dictSort, dictLabel, dictValue } = this.form;
            const form = { id, status, dictSort, dictLabel, dictValue };
