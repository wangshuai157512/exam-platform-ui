...
<script>
import { getManualQualifiedPages } from "@/api/exam/statistics";
...
export default {
  ...
  methods: {
    getTableList(num) {
      getManualQualifiedPages({
        ...this.queryParams,
        startDate: this.ksrq ? this.ksrq[0] : "",
        endDate: this.ksrq ? this.ksrq[1] : ""
      }).then((res) => {
        this.tableList = res.data;
        const nameArray = res.data.map((item) => item.kscx);
        const dataArray = res.data.map((item) => Number(item.ppl));
